...
// Função para verificar se o jogador acertou na dezena do número sorteado
int acertouDezena(int numeroApostado, int numeroSorteado) {
    return (numeroApostado / 10) == (numeroSorteado / 10);
}

// Função para verificar se o jogador acertou na unidade do número sorteado
int acertouUnidade(int numeroApostado, int numeroSorteado) {
    return (numeroApostado % 10) == (numeroSorteado % 10);
}

// Função para verificar se o jogador acertou na soma dos dígitos do número sorteado
...
int main() {
    ...
    while (1) {
        ...
        if (numeroApostado == numeroSorteado) {
            ...
            printf("Jogador acertou no número sorteado ganhou: KZ %.2f\n", (float)premio);
            totalLucro += premio;
        } else if (numeroApostado == ((numeroSorteado / 10) + (numeroSorteado % 10))) {
            premio = 3 * valorApostado;
            printf("Jogador acertou no número sorteado na ordem inversa ganhou: KZ %.2f\n", (float)premio);
            totalLucro += premio;
        } else if (acertouDezena(numeroApostado, numeroSorteado)) {
            premio = 2 * valorApostado;
            printf("Jogador acertou na dezena do número sorteado ganhou: KZ %.2f\n", (float)premio);
            totalLucro += premio;
        } else if (acertouUnidade(numeroApostado, numeroSorteado)) {
            premio = 2 * valorApostado;
            printf("Jogador acertou na unidade do número sorteado ganhou: KZ %.2f\n", (float)premio);
            totalLucro += premio;
        } else if (acertouSomaDigitos(numeroApostado, numeroSorteado)) {
            premio = 2 * valorApostado;
            printf("Jogador acertou na soma dos dígitos do número sorteado ganhou: KZ %.2f\n", (float)premio);
            totalLucro += premio;
        } else if ((numeroApostado % 2 == 0 && numeroSorteado % 2 == 0) ||
                   (numeroApostado % 2 != 0 && numeroSorteado % 2 != 0)) {
            printf("Jogador acertou na paridade do número sorteado: nao ganhou nem lucrou\n");
        } else {
